...
llm = ChatOllama(model=local_llm, temperature=0, device="cuda" if torch.cuda.is_available() else "cpu")
llm_json_mode = ChatOllama(model=local_llm, temperature=0, format="json", device="cuda" if torch.cuda.is_available() else "cpu")
...
retriever = global_vectorstore.as_retriever(k=3)
...
def run_ai_receptionist(question: str, debug=False) -> str:
    ...
    try:
        detection_response = llm_json_mode.invoke(
    ...
    try:
        ...
        if debug:
            print(f"Key Error during workflow: {e}")
