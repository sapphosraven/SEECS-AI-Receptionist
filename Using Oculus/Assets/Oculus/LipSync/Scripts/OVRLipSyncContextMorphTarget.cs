...
public class OVRLipSyncContextMorphTarget : MonoBehaviour
{
    ...
    [Tooltip("Skinned Mesh Renderer for teeth movement")]
    public SkinnedMeshRenderer teethMeshRenderer = null;
    ...
    // private float[] currentBlendWeights = new float[OVRLipSync.VisemeCount];
    private float[] previousVisemeWeights = new float[OVRLipSync.VisemeCount];

    // Audio delay for syncing
    // public float audioDelay = 0.1f;
    ...
    void Start ()
    {
        ...
        else
        {
            ...
            lipsyncContext.Smoothing = Mathf.Max(1, smoothAmount);
        }
    }
    ...
    void SetVisemeToMorphTarget(OVRLipSync.Frame frame)
{
    for (int i = 0; i < visemeToBlendTargets.Length; i++)
    {
        ...
        {
            float weight = frame.Visemes[i] * 75.0f;

            // Optional: emphasize or adjust specific visemes
            if (i == 4 || i == 7) // p_b_m or w_oo visemes
            {
                weight *= 1.5f;
            }

            weight = Mathf.Clamp(weight, 0.0f, 100.0f);

            // Apply weights to the head mesh
            skinnedMeshRenderer.SetBlendShapeWeight(visemeToBlendTargets[i], weight);

            // Apply weights to the teeth mesh if assigned
            if (teethMeshRenderer != null)
            {
                teethMeshRenderer.SetBlendShapeWeight(visemeToBlendTargets[i], weight);
            }
        }
    }
}
    ...
    {   
            ...
            {   if (laughterBlendTarget >= 0 && laughterBlendTarget < skinnedMeshRenderer.sharedMesh.blendShapeCount)
                {
